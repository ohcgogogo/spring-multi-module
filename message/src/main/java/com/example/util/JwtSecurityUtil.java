...
public class JwtSecurityUtil {
    ...
    public static String resolveToken(StompHeaderAccessor accessor) {
        if(accessor.getNativeHeader(HttpHeaders.AUTHORIZATION).size() == 0) {
//                throw new MessageDeliveryException("UNAUTHORIZED");
            throw new MessageDeliveryException("No authorization header");
        }
        log.info("{} : {}", Thread.currentThread().getStackTrace()[1].getMethodName(), "authorization header check");
        String bearerToken = accessor.getNativeHeader("Authorization").get(0).trim();
